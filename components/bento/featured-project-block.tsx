...
const statusStyle: Record<ProjectStatus, { color: string; bgColor: string }> = {
  active: { color: "text-green-500", bgColor: "bg-green-400" },
  maintenance: { color: "text-[#FCA311]", bgColor: "bg-[#FCA311]" },
  archived: { color: "text-gray-500", bgColor: "bg-gray-400" },
}
...
export function FeaturedProjectBlock({ slug, title, status, featuredProjectLabel, viewProjectLabel, statusLabel }: FeaturedProjectBlockProps): React.ReactElement {
  ...
  return (
    <Link href={`/proyectos/${slug}`} className="group block h-full focus-visible:outline-2 focus-visible:outline-[#FCA311] focus-visible:outline-offset-2 focus-visible:rounded-xl">
      <div className="flex h-full min-h-[200px] flex-col rounded-xl border border-white/5 bg-[#222222] p-6 transition-all duration-300 hover:-translate-y-1 hover:border-[#FCA311]/30 hover:shadow-[0_0_30px_rgba(252,163,17,0.05)]">
        {/* Header */}
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="mt-1 flex items-center gap-2">
              <span className="relative flex h-2 w-2">
                {status === "active" && (
                  <span className={`absolute inline-flex h-full w-full animate-ping rounded-full ${style.bgColor} opacity-75`} />
                )}
                <span className={`relative inline-flex h-2 w-2 rounded-full ${style.bgColor}`} />
              </span>
        ...
        <div className="mt-4 flex items-center gap-2 border-t border-white/5 pt-4">
          <span className="text-sm font-medium text-[#FCA311]">{viewProjectLabel}</span>
