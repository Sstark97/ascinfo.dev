    ...
    <Link href={`/blog/${slug}`} className="group block">
      <article className="flex flex-col gap-4 rounded-xl border border-white/5 bg-[#222222] p-5 transition-all duration-300 hover:border-[#fca311]/30 hover:shadow-[0_0_30px_rgba(252,163,17,0.05)] md:flex-row md:items-start">
        ...
        <div className="shrink-0 md:w-24">
          <time className="font-mono text-xs text-[#888888]">{date}</time>
          <p className="mt-1 font-mono text-xs text-[#666666]">{readingTime}</p>
        </div>
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-2 line-clamp-2 text-sm leading-relaxed text-[#888888]">{excerpt}</p>
        </div>
        ...
        <div className="hidden shrink-0 items-center md:flex">
          <ArrowRight className="h-4 w-4 text-[#888888] transition-all duration-200 group-hover:translate-x-1 group-hover:text-[#fca311]" />
        </div>
      </article>
    </Link>
  )
